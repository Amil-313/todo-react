...
function Project({ item, onClear, setValueUpdate, valueUpdate, projects }) {

    const [update, setUpdate] = React.useState(false);

    const onChangeProject = (e) => {
        e.preventDefault();
        projects.map((project) => project.id === valueUpdate.id ? project.title = valueUpdate.title : project);
        setUpdate(false);
    }

  return (
    <>
      {!update ?  <div className='project_name'>
                <p>{item.title}</p>
                <div className="change">
                    <img className='edit' onClick={() => setUpdate(true)} src={require('./img/edit.png')} alt="edit" />
                    <svg className='delete' onClick={() => onClear(item)} fill="#000000" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 48 48" width="48px" height="48px">
            ...
            </div> : <form className='change_project' onSubmit={onChangeProject}>
              <input defaultValue={item.title} type="text" onChange={(e) => setValueUpdate({
                  id: item.id,
                  title: e.target.value
              })} />
              <button>
              <img onClick={() => setUpdate(true)} src={require('./img/done.png')} alt="done" />
              </button>
            </form>}
